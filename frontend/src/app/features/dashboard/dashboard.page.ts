...
@Component({
  ...
  template: `
    <main class="dashboard dashboard-pro" *ngIf="state$ | async as state">
      ...
      <section class="panel dashboard-income-summary">
        <div>
        ...
        <div class="dashboard-income-readonly">
          <article>
            <span>{{ i18n.t('accounts.countryGermany') }} · EUR</span>
            <strong>{{ plannedIncome(state.incomeSummaries, 'DE', 'EUR') | currency: 'EUR' : 'symbol' : '1.2-2' }}</strong>
          </article>
          <article>
            <span>{{ i18n.t('accounts.countryColombia') }} · COP</span>
            <strong>{{ plannedIncome(state.incomeSummaries, 'CO', 'COP') | currency: 'COP' : 'symbol' : '1.2-2' }}</strong>
          </article>
        </div>
      </section>
...
export class DashboardPage {
  ...
  plannedIncomeByCurrency(summaries: DashboardBudgetSummary[], currencyCode: string): number {
    return summaries
      .filter((summary) => summary.currencyCode === currencyCode)
      .reduce((total, summary) => total + this.money(summary.incomeAmount), 0);
  }
